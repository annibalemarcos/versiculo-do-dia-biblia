...
def run_seed():
    print("Iniciando criação de tabelas e carga inicial do banco de dados...")
    ...
    try:
        ...
        if not app:
            app = App(
                ...
                package_id="com.aistudio.biblia.nvgtnt",
                platform="android",
                status="active",
            ...
            cfg = AppConfig(
                ...
                latest_version=1,
                force_update=False,
                store_url="https://play.google.com/store/apps/details?id=com.aistudio.biblia.nvgtnt",
            ...
            flags = [
                ...
                FeatureFlag(app_id=app.id, key="feature_dark_mode", enabled=True, description="Tema escuro completo"),
            ]
            db.add_all(flags)
            print("✓ App e Feature Flags cadastrados.")

        # 2. Permissions
        permissions_data = [
            ("dashboard.read", "Visualizar Dashboard", "dashboard", "Acesso aos relatórios e gráficos executivos"),
            ("content.read", "Visualizar Conteúdo", "content", "Ler versículos, temas, emoções e devocionais"),
            ("content.write", "Criar/Editar Conteúdo", "content", "Publicar e editar versículos e devocionais"),
            ("content.delete", "Excluir Conteúdo", "content", "Remover versículos ou devocionais"),
            ("monetization.read", "Visualizar Monetização", "monetization", "Ver produtos, anúncios e simulações"),
            ("monetization.write", "Gerenciar Monetização", "monetization", "Editar preços, produtos e posicionamentos de anúncio"),
            ("users.read", "Visualizar Usuários", "users", "Acessar listagem e perfil de usuários"),
            ("users.block", "Bloquear Usuários", "users", "Bloquear ou desbloquear contas"),
            ("users.premium_override", "Alterar Status Premium", "users", "Conceder ou revogar assinatura manualmente"),
            ("apps.read", "Visualizar Apps", "apps", "Ver configurações de aplicativos"),
            ("apps.write", "Gerenciar Apps", "apps", "Alterar versões mínimas e modo manutenção"),
            ("feature_flags.read", "Visualizar Feature Flags", "feature_flags", "Ver status das flags"),
            ("feature_flags.write", "Gerenciar Feature Flags", "feature_flags", "Ligar/desligar feature flags"),
            ("notifications.read", "Visualizar Notificações", "notifications", "Ver templates e histórico de push"),
            ("notifications.write", "Criar Notificações", "notifications", "Criar templates de push"),
            ("notifications.send", "Disparar Push", "notifications", "Agendar ou enviar campanhas push"),
            ("experiments.read", "Visualizar Testes A/B", "experiments", "Acessar métricas de experimentos"),
            ("experiments.write", "Gerenciar Testes A/B", "experiments", "Criar e pausar experimentos"),
            ("analytics.read", "Visualizar Telemetria", "analytics", "Consultar logs e métricas agregadas"),
            ("audit.read", "Visualizar Logs de Auditoria", "audit", "Acessar histórico de ações administrativas"),
            ("system.health", "Inspecionar Saúde do Sistema", "system", "Acessar diagnóstico de infraestrutura"),
            ("admins.manage", "Gerenciar Administradores", "admins", "Criar e editar administradores e perfis"),
            ("staff.view", "Visualizar Equipe", "staff", "Visualizar membros da equipe e organograma"),
            ("staff.request_create", "Solicitar Membro de Equipe", "staff", "Solicitar criação de novo membro para o departamento"),
            ("staff.request_role_change", "Solicitar Mudança de Função", "staff", "Solicitar alteração de cargo ou departamento"),
            ("staff.request_promotion", "Solicitar Promoção", "staff", "Solicitar promoção para liderança/master"),
            ("staff.approve_changes", "Aprovar Alterações de Equipe", "staff", "Aprovar ou rejeitar solicitações de equipe"),
            ("staff.manage_department", "Gerenciar Departamento", "staff", "Gestão de membros do próprio departamento"),
            ("tickets.view", "Visualizar Tickets de Suporte", "tickets", "Ver lista e detalhes de tickets"),
            ("tickets.create", "Criar Tickets", "tickets", "Abrir chamados internamente"),
            ("tickets.reply", "Responder Tickets", "tickets", "Enviar mensagens públicas aos usuários"),
            ("tickets.assign", "Atribuir Responsável", "tickets", "Atribuir chamados a membros da equipe"),
        ...
        roles_data = [
            # ID, Name, Desc, System, Dept, Level, IsMaster, CanManageStaff, ReqApproval, ParentRole, Permissions
            ...
            ("support", "Agente de Suporte", "Atendimento a chamados e suporte a usuários", True, "support", 1, False, False, True, "support_master", [
                "dashboard.read", "users.read",
                "tickets.view", "tickets.reply", "tickets.add_participant", "tickets.edit", "tickets.resolve", "tickets.internal_notes",
                "notifications.view", "notifications.mark_read"
            ]),
        ]
        ...
        # 4. Super Admin User
        admin_user = db.query(AdminUser).filter(AdminUser.email == "admin@versiculododia.com").first()
        if not admin_user:
            admin_user = AdminUser(
                email="admin@versiculododia.com",
            ...
            print("✓ Usuário Super Admin inicial criado: admin@versiculododia.com / Admin@123456")
        else:
        ...
        # 16. Analytics Daily Aggregates (last 30 days)
        ...
        # 16. Support Tickets Initial Seed
        ...
        # 16. Broadcast User Notifications (Central de Notificações)
        ...
        if db.query(Banner).count() == 0:
            sample_banners = [
                Banner(
                    id="banner_welcome_premium",
                    app_id="verse_daily",
                    title="Desbloqueie Todos os Planos Devocionais",
                    subtitle="Acesso ilimitado a comentários teológicos e narração com áudio HD.",
                    description="Experimente 7 dias grátis sem anúncios e aprofunde sua fé diariamente.",
                    badge_text="OFERTA",
                    placement="home_top",
                    ...
                    secondary_action_label="Agora não",
                    priority=90,
                    is_active=True,
                    dismissible=True,
                    bg_color="#1E3A8A",
                ...
                Banner(
                    id="banner_reading_audio",
                    app_id="verse_daily",
                    title="Nova Narração Bíblica Disponível",
                    subtitle="Ouça a Palavra com trilha orquestrada e voz suave.",
                    badge_text="NOVO",
                    placement="reading_screen",
                    target_audience="all",
                    action_type="open_url",
                    action_url="https://biblia.app/audio-guide",
                    action_label="Ouvir Agora",
                    priority=50,
                    is_active=True,
                    dismissible=True
                ),
                Banner(
                    id="banner_community_prayer",
                    app_id="verse_daily",
                    title="Participe do Círculo de Oração Semanal",
                    subtitle="Centenas de irmãos em oração toda quarta-feira às 20h.",
                    badge_text="COMUNIDADE",
                    placement="home_middle",
                    target_audience="registered_only",
                    action_type="show_modal",
                    action_url="modal_prayer_circle",
            ...
            for b in sample_banners:
                db.add(b)
            print("✓ Banners de demonstração cadastrados com sucesso.")

        # 16. Dynamic Registration Field Definitions
        if db.query(RegistrationFieldDefinition).count() == 0:
            default_fields = [
                RegistrationFieldDefinition(
                    ...
                    help_text="Usado apenas para comunicações importantes e suporte de oração.",
                    field_type="phone",
                    is_required=False,
                    is_active=True,
                    min_length=8,
                ...
                RegistrationFieldDefinition(
                    ...
                    help_text="Para enviarmos uma mensagem bíblica especial no seu aniversário!",
                    field_type="date",
                    is_required=False,
                    is_active=True,
                    display_order=2,
                ...
                RegistrationFieldDefinition(
                    id="field_church",
                    app_id="verse_daily",
                    field_key="church_community",
                    label="Igreja / Congregação Local",
                    placeholder="Ex: Igreja Batista Central, Assembleia de Deus...",
                    help_text="Opcional. Permite personalizar planos de leitura de comunidades parceiras.",
                    field_type="text",
                    is_required=False,
                    is_active=True,
                    max_length=150,
                ...
                RegistrationFieldDefinition(
                    ...
                    help_text="Para eventos e devocionais locais.",
                    field_type="select",
                    options=[
                        ...
                        {"value": "OUTRO", "label": "Outro Estado / Exterior"}
                    ],
                    is_required=False,
                    is_active=True,
                    display_order=4,
                ...
                RegistrationFieldDefinition(
                    id="field_interests",
                    app_id="verse_daily",
                    field_key="reading_interests",
                    label="Temas de Interesse Espiritual",
                    placeholder="Selecione os temas que mais te tocam",
                    help_text="Recomendamos devocionais e versículos baseados nas suas escolhas.",
                    field_type="multiselect",
                    options=[
                        {"value": "paz_ansiedade", "label": "Paz interior & Superar a Ansiedade"},
                        {"value": "familia_casamento", "label": "Família & Relacionamentos"},
                        {"value": "fe_esperanca", "label": "Fortalecimento da Fé & Esperança"},
                        {"value": "estudo_profundo", "label": "Estudo Bíblico Teológico Profundo"},
                        {"value": "jovens_proposito", "label": "Jovens & Descoberta de Propósito"}
                    ],
                    is_required=False,
                    is_active=True,
                    display_order=5,
            ...
            for f in default_fields:
                db.add(f)
            print("✓ Campos dinâmicos de cadastro cadastrados com sucesso.")

        db.commit()
        print("✓ Carga inicial do banco de dados concluída com sucesso total!")
