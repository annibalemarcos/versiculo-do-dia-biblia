...
from app.services.analytics_service import get_dashboard_metrics, get_user_engagement_metrics
...
def get_admin_user_engagement(
    app_id: str = Query("verse_daily"),
    window: str = Query("30m", enum=["30m", "24h", "7d"]),
    admin = Depends(require_permission("dashboard.read")),
    db: Session = Depends(get_db)
):
    """
    Returns real-time active users and trending verse shares for the User Engagement section.
    """
    data = get_user_engagement_metrics(
        db=db,
        app_id=app_id,
        window=window
    )
