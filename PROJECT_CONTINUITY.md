...
## Validação local pós-importação — 2026-09-14

Após a importação controlada da Fase 2 — Etapa 1 para o repositório oficial, foram executadas validações locais adicionais:

### Backend
- `python -m pytest -v`
- Resultado: **13 passed, 0 failed**
- Foi ajustado o teste de erro canônico de suporte para utilizar corretamente o fixture de banco de testes.
- Foram corrigidas mensagens UTF-8 user-facing no fluxo de suporte.

### Admin Web
- `npm run build`
- Resultado: **BUILD OK**
- Foram restauradas as interfaces de engagement já existentes no projeto:
  - `RealtimeActivePoint`
  - `TrendingVerseShareItem`
  - `ShareChannelBreakdown`
  - `UserEngagementData`
- Isso preservou o dashboard existente sem remover os novos tipos de governança remota.
...
- `:app:assembleRelease`
- Resultado: **BUILD SUCCESSFUL**
- Artefato release local gerado como:
  - `app-release-unsigned.apk`

### Assinatura Android
A configuração de signing foi endurecida:
- Debug continua usando a chave debug padrão quando `debug.keystore` customizado não existir.
- Release **não faz fallback para assinatura debug**.
- Quando a chave de produção não estiver disponível localmente, o release é compilado sem assinatura.
- `:app:signingReport` confirmou:
  - `debug`: chave Android Debug padrão
  - `release`: `Config: null`, `Store: null`, `Alias: null`

### Gradle Wrapper
Foram restaurados os arquivos necessários para builds reproduzíveis sem Gradle global:
- `gradlew`
- `gradlew.bat`
- `gradle/wrapper/gradle-wrapper.jar`

O `gradle-wrapper.properties` existente permanece apontando para Gradle 9.3.1.

### Estado da Fase 2 — Etapa 1
A etapa está validada localmente nas três camadas:
- Backend
- Admin Web
- Android

Próximo passo após commit/push:
- deploy automático do backend no Railway;
- aplicação da migration `007_app_remote_governance`;
- validação do endpoint público de configuração;
- validação do painel administrativo em produção;
- teste funcional das flags de governança no app.
