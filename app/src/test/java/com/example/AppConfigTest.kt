...
class AppConfigTest {
    ...
    @Test
    fun `test official identity and billing defaults`() {
        assertEquals("verse_daily", AppConfig.APP_ID)
        assertEquals("com.aistudio.biblia.nvgtnt", AppConfig.PACKAGE_ID)
        assertEquals(1, AppConfig.VERSION_CODE)

        val purchaseReq = com.example.data.remote.VerifyPurchaseRequest(
            productId = "prod_premium_monthly",
            purchaseToken = "test_token"
        )
        assertEquals("com.aistudio.biblia.nvgtnt", purchaseReq.packageName)
    }
}
