            ...
            <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-bold text-white flex items-center gap-2">
                  ...
                  <span>Campos de Cadastro Dinâmicos (Registration Fields)</span>
                </h3>
                <button
                  onClick={() => navigate('/registration-fields')}
                  className="text-xs text-brand-400 hover:text-brand-300 transition-colors flex items-center gap-1"
                >
                  <span>Gerenciar Definições</span>
              ...
              {user.custom_fields && Object.keys(user.custom_fields).length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {Object.entries(user.custom_fields).map(([key, val]) => (
                    <div
                      key={key}
                    ...
                    >
                      ...
                      <div className="text-xs text-slate-200 font-medium break-words">
                        {val === null || val === undefined || val === '' ? (
                          <span className="text-slate-500 italic">Não preenchido</span>
                        ) : typeof val === 'boolean' ? (
                          <span
                          ...
                          <div className="flex flex-wrap gap-1 mt-1">
                            {val.map((item, idx) => (
                              <span
                                key={idx}
                                className="px-2 py-0.5 rounded bg-brand-500/10 text-brand-300 border border-brand-500/20 text-[10px]"
                              >
                                {String(item)}
                              </span>
                            ))}
                ...
                <div className="p-4 rounded-xl bg-slate-950/40 border border-slate-800/60 text-xs text-slate-400 flex items-center justify-between">
                  <span>Nenhum campo de cadastro opcional ou customizado foi preenchido por este usuário.</span>
                  <span className="text-[10px] text-slate-500 font-mono">custom_fields: {'{}'}</span>
                </div>
              )}
            </div>
