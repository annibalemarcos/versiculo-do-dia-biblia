...
export const MonetizationHub: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    'overview' | 'simulator' | 'products' | 'placements' | 'subscriptions' | 'diagnostic' | 'test_environment'
  >(() => {
    try {
      const p = new URLSearchParams(window.location.search).get('tab');
      if (p === 'test_environment' || p === 'products' || p === 'simulator' || p === 'placements' || p === 'subscriptions' || p === 'diagnostic') {
        return p as any;
      }
    } catch (_) {}
    return 'overview';
  ...
  // Test Environment & Payment Systems Testing State
  const [testEnvConfig, setTestEnvConfig] = useState<{
    enabled: boolean;
    payment_testing_enabled: boolean;
    app_mode: 'TEST' | 'PRODUCTION';
    title: string;
    description: string;
    button_activate_text: string;
    button_expire_text: string;
    button_reset_text: string;
    default_duration_hours: number;
    visible_to: 'all' | 'debug_only' | 'test_accounts';
  }>({
    enabled: true,
  ...
  const [savingTestEnv, setSavingTestEnv] = useState<boolean>(false);
  const [savingPaymentTesting, setSavingPaymentTesting] = useState<boolean>(false);
  const [simulatingTestAction, setSimulatingTestAction] = useState<string | null>(null);
  const [simulationResultDetail, setSimulationResultDetail] = useState<any>(null);
  ...
  const fetchTestEnvConfig = async () => {
    setLoadingTestEnv(true);
    const res = await api.get<any>(`/admin/monetization/test-environment?app_id=${currentAppId}`);
    setLoadingTestEnv(false);
    if (res.success && res.data) {
      const data = res.data;
      setTestEnvConfig({
        ...data,
        payment_testing_enabled: data.payment_testing_enabled !== undefined
          ? Boolean(data.payment_testing_enabled)
          : data.app_mode === 'TEST',
        app_mode: data.app_mode || (data.payment_testing_enabled ? 'TEST' : 'PRODUCTION'),
      });
    }
  };

  const handleSaveTestEnvConfig = async (overrideData?: Partial<typeof testEnvConfig>) => {
    setSavingTestEnv(true);
    const payload = { ...testEnvConfig, ...(overrideData || {}) };
    const res = await api.put<any>(`/admin/monetization/test-environment?app_id=${currentAppId}`, payload);
    setSavingTestEnv(false);
    if (res.success) {
      if (res.data) {
        const data = res.data;
        setTestEnvConfig({
      ...
      showToast('Configurações do Ambiente de Testes salvas com sucesso!', 'success');
    } else {
      showToast(res.error?.message || 'Falha ao salvar configurações do ambiente de testes.', 'error');
    }
  };

  const handleTogglePaymentTesting = async (forcedNewValue?: boolean) => {
    const nextTesting = forcedNewValue !== undefined ? forcedNewValue : !testEnvConfig.payment_testing_enabled;
    const nextMode: 'TEST' | 'PRODUCTION' = nextTesting ? 'TEST' : 'PRODUCTION';
    setSavingPaymentTesting(true);

    // Optimistic UI state update
    setTestEnvConfig((prev) => ({
      ...prev,
      payment_testing_enabled: nextTesting,
      app_mode: nextMode,
    }));

    try {
      const res = await api.put<any>(`/admin/monetization/payment-testing-status?app_id=${currentAppId}`, {
        payment_testing_enabled: nextTesting,
        app_mode: nextMode,
      });
      ...
      if (res.success) {
        ...
        showToast(
          nextTesting
            ? 'Status de teste de pagamentos ATIVADO no backend (Modo Sandbox / Simulação).'
            : 'Status de teste de pagamentos DESATIVADO no backend (Modo Produção / Google Play Live).',
          'success'
        );
        fetchOverview();
      } else {
        ...
        setTestEnvConfig((prev) => ({
          ...prev,
          payment_testing_enabled: !nextTesting,
          app_mode: !nextTesting ? 'TEST' : 'PRODUCTION',
        }));
        showToast(res.error?.message || 'Falha ao atualizar status de teste de pagamento.', 'error');
      }
    } catch (err: any) {
      setTestEnvConfig((prev) => ({
        ...prev,
      ...
      showToast('Erro de conexão ao persistir status de teste.', 'error');
    } finally {
      setSavingPaymentTesting(false);
    }
  };

  const handleRunAdminSimulation = async (action: 'activate' | 'expire' | 'reset', productId = 'premium_yearly') => {
    setSimulatingTestAction(action);
    const res = await api.post<any>(`/admin/monetization/test-environment/simulate?app_id=${currentAppId}`, {
      action,
      product_id: productId,
      duration_hours: testEnvConfig.default_duration_hours,
    });
    setSimulatingTestAction(null);
    if (res.success) {
      setSimulationResultDetail(res.data);
      showToast(res.message || 'Ação de simulação executada com sucesso!', 'success');
      fetchHubData();
      fetchOverview();
    } else {
      showToast(res.error?.message || 'Falha ao executar simulação.', 'error');
    }
  };
  ...
  useEffect(() => {
    const handlePopState = () => {
      try {
        ...
        if (tab && ['overview', 'simulator', 'products', 'placements', 'subscriptions', 'diagnostic', 'test_environment'].includes(tab)) {
  ...
  const handleSaveProduct = async (e: React.FormEvent) => {
    ...
    if (isEditingProduct) {
      ...
      const res = await api.put<PremiumProductItem>(`/admin/monetization/products/${currentProductForm.id}`, {
      ...
      if (res.success) {
        const updated: Partial<PremiumProductItem> = (res.data as any) || {
        ...
        setProducts((prev) =>
          prev.map((p) => (p.id === currentProductForm.id ? { ...p, ...updated } : p))
        );
      ...
      const res = await api.post<PremiumProductItem>('/admin/monetization/products', {
        id: autoId,
        app_id: currentAppId,
      ...
      setSubmittingProduct(false);
      if (res.success) {
        const returnedData = (res.data as any) || {};
        const newProd: PremiumProductItem = {
          id: autoId,
          app_id: currentAppId,
          product_id: currentProductForm.product_id.trim(),
          base_plan_id: currentProductForm.base_plan_id.trim() || null,
        ...
        setProducts((prev) => [newProd, ...prev.filter((p) => p.id !== autoId)]);
        setProductSearch('');
        setProductStatusFilter('all');
        setActiveTab('products');
        setHighlightedProductId(autoId);
        setTimeout(() => setHighlightedProductId(null), 8000);
        showToast(`Novo produto '${newProd.title}' cadastrado e exibido no catálogo!`, 'success');
        setShowProductModal(false);
        fetchHubData();
        fetchOverview();
      } else {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Persistent Payment Systems Testing Status Control Switch Bar */}
      <div
        className={`p-4 sm:p-5 rounded-3xl border transition-all shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4 ${
          testEnvConfig.payment_testing_enabled
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                ...
                {testEnvConfig.payment_testing_enabled
                  ...
                  : 'PRODUÇÃO REAL (GOOGLE PLAY LIVE)'}
              </span>
              <span className="text-[10px] font-mono text-slate-400 bg-slate-900/80 px-2 py-0.5 rounded-md border border-slate-800">
                app_mode: {testEnvConfig.app_mode}
              </span>
            </div>
            <p className="text-xs text-slate-300 mt-1 max-w-2xl">
              {testEnvConfig.payment_testing_enabled
                ? 'Ambiente de testes ativo no backend: simulação de compras autorizada, validação sem cobrança real de cartões e suporte a contas de teste.'
                : 'Ambiente de produção ativo no backend: transações reais da Google Play Store exigidas, verificação rigorosa de recibos criptográficos e cobranças financeiras reais.'}
            </p>
          </div>
        </div>

        {/* Persistent Toggle Switch & Action */}
        <div className="flex items-center gap-3 self-end sm:self-center shrink-0">
          <div className="text-right hidden sm:block">
            <div className="text-[11px] font-semibold text-slate-300">
              {testEnvConfig.payment_testing_enabled ? 'Modo de Teste Habilitado' : 'Modo de Teste Desabilitado'}
            </div>
            <div className="text-[10px] text-slate-500">
              {savingPaymentTesting ? 'Persistindo no backend...' : 'Configuração persistida no backend'}
            </div>
          </div>

          <button
            onClick={() => handleTogglePaymentTesting()}
            disabled={savingPaymentTesting}
            aria-label="Alternar status de teste dos sistemas de pagamento"
            className={`flex items-center gap-2 px-3.5 py-2 rounded-2xl border transition-all cursor-pointer shadow-md select-none ${
              testEnvConfig.payment_testing_enabled
            ...
            } ${savingPaymentTesting ? 'opacity-70 cursor-wait' : 'active:scale-95'}`}
            title="Alternar entre Modo Teste / Sandbox e Produção Real"
          >
            {savingPaymentTesting ? (
            ...
            <span className="text-xs font-bold font-mono">
              {testEnvConfig.payment_testing_enabled ? 'TESTE: ATIVO' : 'PRODUÇÃO: LIVE'}
            </span>
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-slate-800 pb-3">
        ...
        <button
          onClick={() => {
            ...
            const url = new URL(window.location.href);
            url.searchParams.set('tab', 'placements');
            window.history.pushState({}, '', url.toString());
          }}
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-colors ${
        ...
        <button
          onClick={() => {
            ...
            const url = new URL(window.location.href);
            url.searchParams.set('tab', 'diagnostic');
            window.history.pushState({}, '', url.toString());
          }}
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-colors ${
        ...
        <button
          onClick={() => {
            setActiveTab('test_environment');
            const url = new URL(window.location.href);
            url.searchParams.set('tab', 'test_environment');
            window.history.pushState({}, '', url.toString());
          }}
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-colors ${
            activeTab === 'test_environment'
        ...
        >
          ...
          >
            {testEnvConfig.payment_testing_enabled ? 'TESTE' : 'LIVE'}
          </span>
        </button>
      </div>
                  ...
                  <div
                    ...
                    className={`bg-slate-900/80 border rounded-3xl p-5 flex flex-col justify-between space-y-4 shadow-xl transition-all relative overflow-hidden group ${
                      highlightedProductId === prod.id
                        ? 'border-brand-400 ring-2 ring-brand-400/40 shadow-brand-500/20'
                        : isActive
                    ...
                    <div>
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          {highlightedProductId === prod.id && (
                            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-brand-500 text-slate-950 font-mono">
                              ★ Novo
                            </span>
                          )}
                        </div>
      ...
      {activeTab === 'diagnostic' && (
        <div className="space-y-4 animate-fade-in">
          <p className="text-xs text-slate-400">
            Status de prontidão técnica dos provedores de receita e configuração de pagamentos.
          </p>

          {/* Active Payment Testing Status Diagnostic Card */}
          <div
          ...
          >
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-xs font-bold text-white uppercase tracking-wider">
                    Modo dos Sistemas de Pagamento (Backend Billing Engine)
                  </span>
                  <span
                  ...
                  >
                    {testEnvConfig.payment_testing_enabled ? 'TESTE / SANDBOX' : 'PRODUÇÃO LIVE'}
                  </span>
                </div>
                <p className="text-xs text-slate-400 mt-0.5">
                  {testEnvConfig.payment_testing_enabled
                    ? 'Transações de teste e simulação liberadas no backend. Requisições de compra não geram débito real.'
                    : 'Transações em produção. O backend exige verificação de assinatura da Google Play Store.'}
                </p>
              </div>
            </div>
            ...
            <button
              onClick={() => handleTogglePaymentTesting()}
              disabled={savingPaymentTesting}
              className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-xl text-xs font-bold border transition-colors shrink-0 ${
                testEnvConfig.payment_testing_enabled
      ...
      {/* ======================================================= */}
      {/* TAB: TEST ENVIRONMENT (Ambiente de Testes)             */}
      {/* ======================================================= */}
      {activeTab === 'test_environment' && (
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-6 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-start gap-4">
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <h3 className="text-base font-bold text-white">
                    Gestão & Controle do Ambiente de Testes
                  </h3>
                  <span
                    className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full uppercase border ${
                      testEnvConfig.enabled
                        ? 'bg-emerald-950 text-emerald-400 border-emerald-800'
                        : 'bg-slate-800 text-slate-400 border-slate-700'
                    }`}
                  >
                    {testEnvConfig.enabled ? 'Ativado no App' : 'Desativado no App'}
                  </span>
                  <span
                    className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full uppercase border ${
                      testEnvConfig.payment_testing_enabled
                  ...
                  >
                    Pagamentos: {testEnvConfig.payment_testing_enabled ? 'Modo Teste' : 'Produção Live'}
                  </span>
                  <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-slate-800 text-slate-300 border border-slate-700">
                    Modo: {testEnvConfig.app_mode}
                  </span>
                </div>
                <p className="text-xs text-slate-300">
                  Controle remoto do banner de homologação e status de teste dos sistemas de pagamento no backend. Permite alternar entre ambiente Sandbox (sem cobrança real) e Produção Google Play Store.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 self-start md:self-auto">
              <button
                onClick={fetchTestEnvConfig}
                disabled={loadingTestEnv}
                className="inline-flex items-center gap-2 bg-slate-950 hover:bg-slate-800 text-slate-300 px-3.5 py-2 rounded-xl text-xs font-semibold border border-slate-800 transition-colors"
              >
                <RefreshCw className={`w-3.5 h-3.5 ${loadingTestEnv ? 'animate-spin text-brand-400' : ''}`} />
                <span>Recarregar</span>
              </button>
              <button
                onClick={() => handleSaveTestEnvConfig()}
                disabled={savingTestEnv}
                className="inline-flex items-center gap-2 bg-amber-500 hover:bg-amber-400 text-slate-950 px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-md shadow-amber-500/20 active:scale-95"
              >
                <Save className="w-3.5 h-3.5" />
                <span>{savingTestEnv ? 'Salvando...' : 'Salvar Alterações'}</span>
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-5 shadow-xl space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => {
                    const next = !testEnvConfig.enabled;
                    setTestEnvConfig({ ...testEnvConfig, enabled: next });
                    handleSaveTestEnvConfig({ enabled: next });
                  }}
                  className={`p-1 rounded-xl transition-colors ${
                    testEnvConfig.enabled ? 'text-emerald-400' : 'text-slate-500'
                  }`}
                >
                  {testEnvConfig.enabled ? (
              ...
              <div>
                <h4 className="text-sm font-bold text-white">
                  {testEnvConfig.enabled ? 'Painel Ativo no Android' : 'Painel Oculto no Android'}
                </h4>
                <p className="text-xs text-slate-400 mt-1">
                  {testEnvConfig.enabled
                    ? 'O card de homologação é renderizado na tela de Paywall do app Android para testes.'
                    : 'O card de homologação está completamente invisível no app Android.'}
                </p>
              </div>

              <div className="pt-2 border-t border-slate-800">
                <span className="text-[11px] font-mono text-slate-500">
                  Status: <strong>{testEnvConfig.enabled ? 'ENABLED (200 OK)' : 'DISABLED (Oculto)'}</strong>
                </span>
              </div>
            </div>

            {/* Card 2: Status de Teste dos Sistemas de Pagamento */}
            <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-5 shadow-xl space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-slate-300 uppercase tracking-wider">
                ...
                <button
                  onClick={() => handleTogglePaymentTesting()}
                  disabled={savingPaymentTesting}
                  aria-label="Alternar modo de teste de pagamentos"
                  className={`p-1 rounded-xl transition-colors cursor-pointer ${
                    testEnvConfig.payment_testing_enabled ? 'text-amber-400' : 'text-slate-500'
                  }`}
                  title="Alternar Status de Teste de Pagamentos"
                >
                  {savingPaymentTesting ? (
                ...
                <div className="flex items-center gap-2">
                  <h4 className="text-sm font-bold text-white">
                    {testEnvConfig.payment_testing_enabled
                      ...
                      : 'Modo Produção Real (Google Play)'}
                  </h4>
                  <span
                    className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase ${
                      testEnvConfig.payment_testing_enabled
                  ...
                  >
                    {testEnvConfig.app_mode}
                  </span>
                </div>
                <p className="text-xs text-slate-400 mt-1">
                  {testEnvConfig.payment_testing_enabled
                    ? 'O backend autoriza simulação de compras e valida assinaturas sem cobrança financeira real de cartões.'
                    : 'O backend exige faturamento comercial autêntico via Google Play Store e valida recibos criptográficos reais.'}
                </p>
              </div>

              <div className="pt-2 border-t border-slate-800 flex gap-2">
                <button
                  onClick={() => handleTogglePaymentTesting(true)}
                  disabled={savingPaymentTesting}
                ...
                <button
                  onClick={() => handleTogglePaymentTesting(false)}
                  disabled={savingPaymentTesting}
                  className={`flex-1 py-1.5 px-3 rounded-xl text-xs font-bold transition-all ${
                    !testEnvConfig.payment_testing_enabled
            ...
            {/* Card 3: Público-Alvo / Visibilidade */}
            <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-5 shadow-xl space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-slate-300 uppercase tracking-wider">
                  Filtro de Audiência
                ...
                <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-slate-800 text-slate-300 uppercase">
                  {testEnvConfig.visible_to}
                </span>
              </div>

              <div>
                <h4 className="text-sm font-bold text-white">Audiência do Painel</h4>
                <p className="text-xs text-slate-400 mt-1">
                  Selecione quem pode visualizar o painel de testes no aplicativo:
                </p>
              </div>

              <div className="space-y-1 pt-1">
                <select
                  value={testEnvConfig.visible_to}
                  onChange={(e) => {
                    const nextVal = e.target.value as any;
                    setTestEnvConfig({ ...testEnvConfig, visible_to: nextVal });
                    handleSaveTestEnvConfig({ visible_to: nextVal });
                  }}
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-amber-500"
                >
                  <option value="all">Todos os Usuários no Paywall (Padrão para Testes)</option>
                  <option value="debug_only">Apenas Builds Debug / Emulador</option>
                  <option value="test_accounts">Apenas Contas de Teste Cadastradas</option>
                </select>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            {/* Form de Edição de Textos e Duração (7 Cols) */}
            <div className="lg:col-span-7 bg-slate-900/80 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-5">
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                <h4 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
                  ...
                  <span>Editar Textos & Parâmetros do Painel no App</span>
                </h4>
                <span className="text-[11px] text-slate-500 font-mono">
                  Sincronização em tempo real via Config API
                </span>
              </div>
              ...
              <div className="space-y-4 text-xs">
                ...
                <div className="space-y-1.5">
                  <label className="text-slate-300 font-medium block">
                    Título do Card no App Android
                  </label>
                  <input
                    type="text"
                    value={testEnvConfig.title}
                    onChange={(e) =>
                      setTestEnvConfig({ ...testEnvConfig, title: e.target.value })
                    }
                    placeholder="Ex: Ambiente de Testes: Simulação de Assinatura"
                ...
                <div className="space-y-1.5">
                  <label className="text-slate-300 font-medium block">
                    Texto Explicativo no App Android
                  </label>
                  <textarea
                    rows={3}
                    value={testEnvConfig.description}
                    onChange={(e) =>
                      setTestEnvConfig({ ...testEnvConfig, description: e.target.value })
                    }
                    placeholder="Ex: Este painel só funciona em ambiente de teste/debug..."
                ...
                {/* Grid de Botões e Duração */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {/* Botão de Ativação */}
                  <div className="space-y-1.5">
                    <label className="text-slate-300 font-medium block">
                      Texto do Botão de Ativação
                    </label>
                    <input
                      type="text"
                      value={testEnvConfig.button_activate_text}
                      onChange={(e) =>
                        setTestEnvConfig({
                          ...testEnvConfig,
                          button_activate_text: e.target.value,
                        })
                      }
                      placeholder="Ex: Ativar (24h)"
                      className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-white font-mono placeholder-slate-600 focus:outline-none focus:border-amber-500"
                    />
                  </div>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex justify-between items-center">
                      <label className="text-slate-300 font-medium">Duração da Assinatura (Horas)</label>
                      <span className="font-mono text-amber-400 font-bold">
                        {testEnvConfig.default_duration_hours}h ({Math.round(testEnvConfig.default_duration_hours / 24)}d)
                      </span>
                    </div>
                    <input
                      type="number"
                      min={1}
                      max={720}
                      value={testEnvConfig.default_duration_hours}
                      onChange={(e) =>
                        setTestEnvConfig({
                          ...testEnvConfig,
                          default_duration_hours: Number(e.target.value),
                        })
                      }
                    ...
                    <div className="flex gap-1.5 pt-1">
                      {[1, 24, 48, 72, 168].map((h) => (
                        <button
                          key={h}
                          type="button"
                          onClick={() => setTestEnvConfig({ ...testEnvConfig, default_duration_hours: h })}
                          className={`px-2 py-0.5 rounded text-[10px] font-mono transition-colors ${
                            testEnvConfig.default_duration_hours === h
                  ...
                  <div className="space-y-1.5">
                    <label className="text-slate-300 font-medium block">
                      Texto do Botão de Expiração
                    </label>
                    <input
                      type="text"
                      value={testEnvConfig.button_expire_text}
                      onChange={(e) =>
                        setTestEnvConfig({
                          ...testEnvConfig,
                          button_expire_text: e.target.value,
                        })
                      }
                      placeholder="Ex: Expirar"
                      className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-white font-mono placeholder-slate-600 focus:outline-none focus:border-amber-500"
                    />
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      type="text"
                      value={testEnvConfig.button_reset_text}
                      onChange={(e) =>
                        setTestEnvConfig({
                          ...testEnvConfig,
                          button_reset_text: e.target.value,
                        })
                      }
                      placeholder="Ex: Resetar para Gratuito"
                      className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-white font-mono placeholder-slate-600 focus:outline-none focus:border-amber-500"
                    />
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-slate-800 flex justify-end gap-2">
                  <button
                    ...
                    onClick={() => {
                      setTestEnvConfig({
                        ...
                        title: 'Ambiente de Testes: Simulação de Assinatura',
                        description:
                          'Este painel só funciona em ambiente de teste/debug e valida a autoridade do servidor sem burlar a Google Play em produção.',
                        button_activate_text: 'Ativar (24h)',
                        button_expire_text: 'Expirar',
                        button_reset_text: 'Resetar para Gratuito',
                        default_duration_hours: 24,
                  ...
                  <button
                    ...
                    onClick={() => handleSaveTestEnvConfig()}
                    disabled={savingTestEnv}
                    className="px-5 py-2 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold rounded-xl text-xs flex items-center gap-2 shadow-md shadow-amber-500/20"
                  >
                    <Save className="w-3.5 h-3.5" />
                    <span>{savingTestEnv ? 'Salvando...' : 'Salvar Alterações'}</span>
                  </button>
            ...
            {/* Live Preview do Card no App Android (5 Cols) */}
            <div className="lg:col-span-5 space-y-4">
              <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-5 shadow-xl space-y-4">
                <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                  <h4 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-2">
                    <Sparkles className="w-4 h-4 text-brand-400" />
                    <span>Pré-Visualização no App Android</span>
                  </h4>
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-slate-950 text-slate-400 font-mono">
                    Paywall Screen Preview
                  </span>
                </div>
                ...
                <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800 space-y-4">
                  ...
                  <div className="flex items-center justify-between text-[11px] text-slate-500 border-b border-slate-800/60 pb-2">
                    <span>Versículo do Dia Premium</span>
                    <Crown className="w-3.5 h-3.5 text-amber-400" />
                  </div>

                  {/* Rendered Test Card in App */}
                  {testEnvConfig.enabled ? (
                    <div className="p-4 rounded-2xl bg-amber-950/20 border border-amber-500/30 space-y-3">
                      <div className="flex items-center gap-2">
                        <Bug className="w-4 h-4 text-amber-400 shrink-0" />
                        <span className="text-xs font-bold text-amber-300">
                          {testEnvConfig.title || 'Ambiente de Testes: Simulação de Assinatura'}
                        </span>
                      </div>

                      <p className="text-[11px] text-slate-300 leading-relaxed">
                        {testEnvConfig.description ||
                          'Este painel só funciona em ambiente de teste/debug e valida a autoridade do servidor...'}
                      </p>

                      <div className="flex items-center gap-1.5 flex-wrap pt-1">
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-amber-500 text-slate-950 shadow-sm">
                          {testEnvConfig.button_activate_text || 'Ativar (24h)'}
                        </span>
                        <span className="text-[10px] font-semibold px-2 py-0.5 rounded bg-slate-800 text-slate-300 border border-slate-700">
                          {testEnvConfig.button_expire_text || 'Expirar'}
                        </span>
                        <span className="text-[10px] font-semibold px-2 py-0.5 rounded bg-slate-800 text-slate-300 border border-slate-700">
                          {testEnvConfig.button_reset_text || 'Resetar para Gratuito'}
                        </span>
                      </div>
                    </div>
                  ) : (
                    <div className="p-8 text-center rounded-2xl bg-slate-900/60 border border-slate-800/60 text-slate-500 space-y-1">
                      <Lock className="w-6 h-6 mx-auto text-slate-600 mb-2" />
                      <p className="text-xs font-semibold text-slate-400">Card Desativado</p>
                      <p className="text-[11px]">
                        O card não será exibido para nenhum usuário no aplicativo Android.
                      </p>
                    </div>
                  )}

                  {/* Other Paywall Mock items */}
                  <div className="p-3 bg-slate-900 rounded-xl border border-slate-800 flex justify-between items-center opacity-40">
                    <span className="text-xs text-white font-medium">Assinatura Anual Premium</span>
                    <span className="text-xs text-brand-400 font-bold font-mono">R$ 99,90/ano</span>
                  </div>
                </div>
              </div>

              {/* Trigger Direct Simulation from Dashboard */}
              <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-5 shadow-xl space-y-3">
                <h4 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-2">
                  <Play className="w-4 h-4 text-emerald-400" />
                  <span>Simulador Imediato no Servidor</span>
                </h4>
                <p className="text-xs text-slate-400">
                  Dispare chamadas de simulação diretamente na API para testar se o banco de dados e as permissões de assinatura são atribuídas corretamente:
                </p>

                <div className="flex flex-wrap gap-2 pt-1">
                  <button
                    onClick={() => handleRunAdminSimulation('activate')}
                    disabled={simulatingTestAction !== null}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold transition-all shadow-md shadow-emerald-600/20 disabled:opacity-50"
                  >
                    <CheckCircle2 className="w-3.5 h-3.5" />
                    <span>
                      {simulatingTestAction === 'activate' ? 'Ativando...' : 'Ativar Assinatura Simulada'}
                    </span>
                  </button>

                  <button
                    onClick={() => handleRunAdminSimulation('expire')}
                    disabled={simulatingTestAction !== null}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-600 hover:bg-amber-500 text-white text-xs font-bold transition-all disabled:opacity-50"
                  >
                    <Clock className="w-3.5 h-3.5" />
                    <span>
                      {simulatingTestAction === 'expire' ? 'Expirando...' : 'Forçar Expiração'}
                    </span>
                  </button>

                  <button
                    onClick={() => handleRunAdminSimulation('reset')}
                    disabled={simulatingTestAction !== null}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-semibold transition-all disabled:opacity-50"
                  >
                    <RefreshCw className="w-3.5 h-3.5" />
                    <span>
                      {simulatingTestAction === 'reset' ? 'Resetando...' : 'Resetar para Gratuito'}
                    </span>
                  </button>
                </div>

                {simulationResultDetail && (
                  <div className="mt-3 p-3 bg-slate-950 rounded-2xl border border-slate-800 font-mono text-[11px] text-slate-300 space-y-1">
                    <div className="flex justify-between">
                      <span className="text-slate-500">Status da Simulação:</span>
                      <strong className="text-emerald-400">{simulationResultDetail.status}</strong>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Expira em:</span>
                      <span className="text-white">
                        {simulationResultDetail.expires_at
                          ? new Date(simulationResultDetail.expires_at).toLocaleString('pt-BR')
                          : 'N/A'}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">ID da Assinatura:</span>
                      <span className="text-brand-400">{simulationResultDetail.subscription_id || '-'}</span>
                    </div>
                  </div>
                )}
              </div>
